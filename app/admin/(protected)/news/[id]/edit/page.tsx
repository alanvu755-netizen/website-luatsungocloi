...
import { Sparkles, Save, CheckCircle, AlertCircle, Check, RefreshCw, X, Newspaper } from "lucide-react";
...
export default function EditNewsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([articleData, menuData, paData, objData]) => {
      ...
      .catch(() => setError("Không thể tải thông tin bài viết tin tức."))
  ...
  const handleGenerateAIDraft = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || "Lỗi tạo nội dung từ AI.");
  ...
  const handleApplyAIDraft = () => {
    ...
    if (aiDraftOutput.suggestedTitle) setTitle(stripHtmlTags(aiDraftOutput.suggestedTitle));
  ...
  const handleSaveNews = async (targetStatus: "DRAFT" | "PUBLISHED" | "HIDDEN") => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Lỗi cập nhật bài viết tin tức.");

      router.push("/admin/news");
    } catch (err: any) {
      setError(err.message || "Lỗi cập nhật bài viết tin tức.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500">Đang tải thông tin bài viết tin tức...</div>;
  }
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Cập nhật nội dung tin tức thời sự pháp luật và truyền thông.
          </p>
        </div>
        ...
        >
          ...
          <span>AI Writer Studio (Tạo/Sửa Tin tức)</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-3">
            ...
            <RichArticleEditor content={content} onChange={setContent} />
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-sm text-navy uppercase">Trợ lý AI Writer (Tạo/Sửa Tin tức)</h2>
                </div>
